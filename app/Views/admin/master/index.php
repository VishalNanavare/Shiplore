        ...
        <?php foreach ($rows as $r): ?>
            <tr>
                ...
                <td class="text-end">
                    ...
                    <form method="post" action="<?= site_url('admin/' . $spec['slug'] . '/' . $r['id'] . '/toggle') ?>" data-ajax-refresh class="d-inline" data-confirm="<?= $isActive ? 'Deactivate' : 'Activate' ?> "<?= esc((string) ($r[($spec['columns'][0][0] ?? 'name')] ?? ''), 'attr') ?>"?"><?= csrf_field() ?><button class="btn btn-sm <?= $isActive ? 'btn-success' : 'btn-outline-secondary' ?>" title="<?= $isActive ? 'Active — click to deactivate' : 'Inactive — click to activate' ?>"><i class="bi bi-toggle-<?= $isActive ? 'on' : 'off' ?>"></i></button></form>
                </td>
            </tr>
        <?php endforeach; ?>
