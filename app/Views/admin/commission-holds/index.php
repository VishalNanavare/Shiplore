...
<div class="row g-3 mb-3">
    <?php foreach (['on_hold', 'accrued', 'settled', 'cancelled', 'reversed'] as $s): ?>
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <span class="fw-semibold">Commission Hold Queue</span>
        <div class="d-flex gap-1">
            <?php foreach (array_keys($tone) as $s): ?>
                <a class="btn btn-sm btn-outline-secondary <?= $status === $s ? 'active' : '' ?>" href="<?= site_url('admin/commission-holds?status=' . $s) ?>"><?= esc(str_replace('_', ' ', $s)) ?></a>
            <?php endforeach; ?>
        </div>
    </div>
    <div class="table-responsive"><table class="table table-hover align-middle mb-0">
        <thead class="table-light"><tr><th>Sub-order</th><th>Vendor</th><th>Lines</th><th>Commission</th><th>Window ends</th><th>Status</th></tr></thead>
        <tbody>
        ...
        <?php endforeach; ?>
        <?php if (empty($rows)): ?><tr><td colspan="6" class="text-center text-secondary py-4">Nothing in this state.</td></tr><?php endif; ?>
        </tbody>
    </table></div>
</div>
<?= $this->endSection() ?>
