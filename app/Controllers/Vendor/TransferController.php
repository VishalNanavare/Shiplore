...
final class TransferController extends BaseVendorController
{
    ...
    {
        return $this->act(fn () => service('transferService')->approve($id, $this->vendorId(), (int) session()->get('user_id')));
    }

    public function reject(int $id): RedirectResponse
    {
        return $this->act(fn () => service('transferService')->reject($id, $this->vendorId(), (int) session()->get('user_id'), (string) $this->request->getPost('reason')));
    }

    public function pack(int $id): RedirectResponse
    {
        return $this->act(fn () => service('transferService')->pack($id, $this->vendorId(), (int) session()->get('user_id')));
    }

    public function dispatch(int $id): RedirectResponse
    {
        return $this->act(fn () => service('transferService')->dispatch($id, $this->vendorId(), (int) session()->get('user_id')));
    }

    public function receive(int $id): RedirectResponse
    {
        return $this->act(fn () => service('transferService')->receive($id, $this->vendorId(), (float) $this->request->getPost('qty_received'), (int) session()->get('user_id')));
    }

    public function close(int $id): RedirectResponse
    {
        return $this->act(fn () => service('transferService')->close($id, $this->vendorId(), (int) session()->get('user_id')));
    }

    public function cancel(int $id): RedirectResponse
    {
        return $this->act(fn () => service('transferService')->cancel($id, $this->vendorId(), (int) session()->get('user_id')));
    }

    /** @param callable():array $fn */
    private function act(callable $fn): RedirectResponse
    {
        if ($denied = $this->requireVendor()) {
            return $denied;
        }
