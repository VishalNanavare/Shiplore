...
final class VendorRegistrationTest extends CIUnitTestCase
{
    ...
    {
        $session = service('session')->get() + ['reg' => $this->fields()];
        $data    = $this->csrf() + ['mobile_otp' => '111111', 'email_code' => '111111'];
        $r       = $this->withSession($session)->post('register/complete', $data);
        $r->assertRedirect();
        $this->assertTrue($this->reg->created);
    }

    public function testWrongOtpStopsRegistration(): void
    {
        $session = service('session')->get() + ['reg' => $this->fields()];
        $data    = $this->csrf() + ['mobile_otp' => '000000', 'email_code' => '111111'];
